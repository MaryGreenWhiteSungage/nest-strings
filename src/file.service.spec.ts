...
describe('FileService', () => {
  ...
  it('should create and remove the file from src and dist should auto update.', async () => {
    ...
    await fileService.saveFile(dto);
  ...
  it('should create and the file and be able to read the exact data back.', async () => {
    const dto: I18nFileDto = {
      stringType: 'test1',
      language: 'es',
      jsonBody: test1,
    ...
    const jsonString: string = await fileService.readFile(
      dto.stringType,
      dto.language,
    );
    expect(jsonString).toEqual(test1);
  });
});
